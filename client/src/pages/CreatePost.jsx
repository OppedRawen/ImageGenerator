...
const CreatePost = ()=>{
    ...
    const handleSubmit = async(e)=>{
        ...
        if(form.prompt&&form.photo){
            setLoading(true);
            try {
                const response = await fetch('https://imagegenerratee.herokuapp.com/api/v1/posts', {
                method: 'POST',
                headers: {
    ...
    const generateImg= async()=>{
        if(form.prompt){
            try{
                ...
                const response = await fetch('https://imagegenerratee.herokuapp.com/api/v1/dalle', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
        ...
        <section className='max-w-7xl mx-auto'>
            ...
            <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
                <div className='flex flex-col gap-5'>
                    ...
                    />
                    {/* container for the AI image */}
                    <div className='relative bg-gray-300 bordr border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center'>
                        {form.photo ? (
                            ...
                            {generatingImg&&(
                        <div className='absolute inset-0 z-0 flex justify-center items-center bg-[rgba(0,0,0,.5) rounded-lg'> <Loader /></div>
                    )}
                    </div>
